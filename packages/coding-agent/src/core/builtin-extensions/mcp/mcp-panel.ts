	...
	private rebuildServerTools(server: ServerState, entry: ServerCacheEntry): void {
		...
		for (const tool of entry.tools ?? []) {
			...
			newTools.push({
				...
				isDirect,
				wasDirect:
					prev !== undefined ? (server.tools.find((t) => t.name === tool.name)?.wasDirect ?? false) : false,
				estimatedTokens: estimateTokens(tool),
			});
		}
		...
		if (server.exposeResources) {
			for (const resource of entry.resources ?? []) {
				...
				newTools.push({
					...
					isDirect,
					wasDirect:
						prev !== undefined ? (server.tools.find((t) => t.name === baseName)?.wasDirect ?? false) : false,
					estimatedTokens: estimateTokens(ct),
				});
			}
		}
		...
		if (this.servers.length === 0) {
			...
			lines.push(
				row(
					fg(
						t.hint,
						italic(this.authOnly ? "No OAuth-capable MCP servers configured." : "No MCP servers configured."),
					),
				),
			);
			lines.push(emptyRow());
