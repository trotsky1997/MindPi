...
import type { ExtensionAPI, ToolInfo } from "../../extensions/types.ts";
import {
	authenticateServer,
	logoutServer,
	openMcpAuthPanel,
...
import { bootstrapDirectToolMetadata, flushMetadataCache, initializeMcp, updateStatusBar } from "./init.ts";
...
export default async function mcpAdapter(pi: ExtensionAPI) {
	...
	// Warm the metadata cache for direct-tool servers missing from it BEFORE we
	// resolve/register tools, so direct tools are available from the first turn
	// and the tool set (part of the prompt-cache prefix) stays fixed for the
	// session. Best-effort; on failure we fall back to deferred registration.
	if (process.env.MCP_DIRECT_TOOLS !== "__none__") {
		try {
			const warmed = await bootstrapDirectToolMetadata(earlyConfig, earlyCache);
			if (warmed.length > 0) earlyCache = loadMetadataCache();
		} catch (error) {
			console.error("MCP: direct-tools metadata bootstrap error", error);
		}
	}
